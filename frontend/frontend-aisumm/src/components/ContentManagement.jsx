...
const ContentManagement = () => {
  ...
  const handleGenerateSummary = async (e) => {
    ...
    try {
      ...
      setSubmitError(err.response?.data?.message || 'Failed to generate summary');
    } finally {
  ...
  const handleDelete = async (summaryId) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/summary/delete/${summaryId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchContents(currentPage);
      if (searchedSummary && searchedSummary._id === summaryId) setSearchedSummary(null);
    } catch (err) {
      ...
      <div style={{
        ...
        {startPage > 1 && (
          <>
            ...
            {startPage > 2 && <span style={{ color: '#94a3b8', fontSize: '18px' }}>⋯</span>}
          </>
        )}
        ...
        {pageNumbers.map(number => (
          <button
            ...
            style={{
              ...
              border: currentPage === number ? 'none' : '1px solid rgba(255, 255, 255, 0.2)',
              borderRadius: '12px',
              cursor: 'pointer',
              fontSize: '14px',
              fontWeight: currentPage === number ? '700' : '600',
              transition: 'all 0.3s ease',
              boxShadow: currentPage === number ? '0 4px 15px rgba(102, 126, 234, 0.3)' : 'none'
            }}
            onMouseEnter={(e) => {
        ...
        {endPage < totalPages && (
          <>
            {endPage < totalPages - 1 && <span style={{ color: '#94a3b8', fontSize: '18px' }}>⋯</span>}
            <button
  ...
  const renderSummaryCard = (item) => (
    <div
      ...
      onMouseEnter={(e) => {
        ...
        e.target.style.boxShadow = '0 20px 40px rgba(0, 0, 0, 0.4), 0 0 0 1px rgba(102, 126, 234, 0.3)';
        e.target.style.border = '1px solid rgba(102, 126, 234, 0.4)';
      }}
      onMouseLeave={(e) => {
        e.target.style.transform = 'translateY(0) scale(1)';
        e.target.style.boxShadow = '0 8px 32px rgba(0, 0, 0, 0.3), 0 0 0 1px rgba(255, 255, 255, 0.05)';
        ...
        <>
          ...
          {loading ? (
            ...
            <div style={{
              ...
              <h3 style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>No content found</h3>
              <p style={{ fontSize: '1rem', opacity: 0.8 }}>Start creating AI-powered summaries to see them here!</p>
            </div>
          ) : (
            <>
              {/* Summary info */}
              <div style={{ 
                ...
                <span>
                  📊 Showing {((pagination.currentPage - 1) * pagination.limit) + 1}-{Math.min(pagination.currentPage * pagination.limit, pagination.totalCount)} of {pagination.totalCount} summaries
                </span>
                <span>
